...
export default function Index() {
  ...
  const userPosition = useSelector(
    (state: TestState) => state.fetcher.userPosition
  );
  const userDepartment = useSelector(
    (state: TestState) => state.fetcher.userDepartment
  );
  ...
  const handleClick = async () => {
    if (position) {
      ...
      if (position === "백엔드") {
        await axios.get(`/backendApplication?sid=${id}`).then(async (res) => {
          // console.log("백엔드", res.data);
          if (
            res.data.motive ||
            res.data.hardWork ||
            res.data.keyWord ||
            res.data.mostDeeplyWork
          ) {
      ...
      if (position === "프론트엔드") {
        await axios.get(`/frontendApplication?sid=${id}`).then(async (res) => {
      ...
      if (position === "디자인") {
        await axios.get(`/designApplication?sid=${id}`).then(async (res) => {
  ...
  const continueApply = async () => {
    ...
    if (position === "백엔드") {
      await axios.get(`/backendApplication?sid=${id}`).then(async (res) => {
        ...
        await dispatch(
          saveBackEnd({
            userDifficultAndOvercoming: res.data.difficultAndOvercoming,
            userImportantGroup: res.data.importantGroup,
            userPortfolioLinkBack: res.data.portfolioLink,
            userStudyFramework: res.data.studyFramework,
          })
        );
        await dispatch(
          saveCommon({
            userMotiv: res.data.motive,
            userHardWork: res.data.hardWork,
            userKeyword: res.data.keyWord,
            userMostDeeplyWork: res.data.mostDeeplyWork,
          })
        );
    ...
    if (position === "프론트엔드") {
      await axios.get(`/frontendApplication?sid=${id}`).then(async (res) => {
        ...
        await dispatch(
          saveFrontEnd({
            userWhyFrontend: res.data.whyFrontend,
            userUsingStack: res.data.usingStack,
            userTeamProject: res.data.teamProject,
            userAchieve: res.data.achieve,
            userPortfolioLinkFront: res.data.portfolioLink,
          })
        );
        await dispatch(
          saveCommon({
    ...
    if (position === "디자인") {
      await axios.get(`/designApplication?sid=${id}`).then(async (res) => {
        ...
        await dispatch(
          saveDesign({
            userWhyDesign: res.data.whyDesign,
            userToolExperience: res.data.toolExperience,
            userTeamworkExperience: res.data.teamworkExperience,
            userPortfolioLinkDesign: res.data.portfolioLink,
            userDesignGrowth: res.data.designGrowth,
          })
        );
        await dispatch(
          saveCommon({
  ...
  const tempContinueApply = async () => {
    setTempCount((prev) => prev + 1);
    if (tempPosition === "백엔드") {
      await axios.get(`/backendApplication?sid=${tempId}`).then(async (res) => {
        await dispatch(
          saveIndex({
            userName: res.data.name,
            userID: res.data.sid,
    ...
    if (tempPosition === "프론트엔드") {
      await axios
        .get(`/frontendApplication?sid=${tempId}`)
        .then(async (res) => {
          await dispatch(
            saveIndex({
              userName: res.data.name,
    ...
    if (tempPosition === "디자인") {
      await axios.get(`/designApplication?sid=${tempId}`).then(async (res) => {
        // console.log("design :", res.data);
        await dispatch(
          saveIndex({
            userName: res.data.name,
  ...
  const tempClick = async () => {
    if (tempPosition) {
      ...
      if (tempPosition === "백엔드") {
        await axios
          .get(
            `/backendApplication/getBackendApplicationWithEmail?email=${tempEmail}&sid=${tempId}`
          )
          .then(async (res) => {
            // console.log("백엔드", res.data);
            if (
      ...
      if (tempPosition === "프론트엔드") {
        await axios
          .get(
            `/frontendApplication/getFrontendApplicationWithEmail?email=${tempEmail}&sid=${tempId}`
          )
          .then(async (res) => {
      ...
      if (tempPosition === "디자인") {
        await axios
          .get(
            `/designApplication/getDesignApplicationWithEmail?email=${tempEmail}&sid=${tempId}`
          )
          .then(async (res) => {
  ...
  const checking = (
    event: React.MouseEvent<HTMLImageElement | HTMLElement>
  ): void => {
  ...
  const changeValue = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.name === "이름") {
      const eventName = event.target.value.replace(
        /[_/]|[0-9]|[ \[\]{}()<>?|`~!@#$%^&*-+=,.;:\"'\\]/g,
        ""
      );
  ...
  if (timeState) {
    ...
    return (
      <Section>
        {itIsTemp ? (
          <Modal
            text="이미 지원한 지원서가 존재해요, 불러오기 버튼을 눌러주세요!"
            imgSrc={human}
            alt="불러오기"
          >
            <Button name="제출하기" onClick={isSubmit}>
              메인 화면으로 이동
            </Button>
          </Modal>
        ) : null}
        {isNotTempState ? (
          <Modal
            text="정보를 제대로 입력해주세요, 발견된 지원서가 없어요!"
            imgSrc={human}
            alt="불러오기"
          >
            <Button name="제출하기" onClick={isSubmit}>
        ...
        {load ? (
          <Modal
            text="작성하신 지원서의 이메일과 학번을 통해 지원서를 불러올 수 있어요"
            imgSrc={isTemp}
            alt="찾기"
          >
            <div
        ...
        {submit ? (
          <Modal
            text="이미 제출한 지원서가 있어요, 결과 발표를 위해 조금만 기다려주세요!"
            imgSrc={human}
            alt="불러오기"
          >
            <Button name="제출하기" onClick={isSubmit}>
        ...
        {temp ? (
          <Modal
            text="앗, 저장된 지원서가 발견되었어요 계속 작성할까요?"
            imgSrc={human}
            alt="불러오기"
          >
            <Button
              name="임시저장"
              onClick={id.length === 9 ? newApply : tempNewApply}
              disabled={tempButtonState}
            >
              {tempCount >= 1 ? `잠시만 기다려주세요...` : `새로 작성하기`}
            </Button>
            <Button
              name="제출하기"
              onClick={id.length === 9 ? continueApply : tempContinueApply}
              disabled={tempButtonState}
            >
              {tempCount >= 1 ? `잠시만 기다려주세요...` : `이어서 작성하기`}
            </Button>
          </Modal>
        ) : null}
        <Banner />
        ...
        <Article>
          ...
          <InputBox
            type="text"
            placeholder="연락 가능한 번호를 입력해주세요 (예시 : 01012345678)"
            name="연락처"
            maxLength={11}
            onChange={changeValue}
            value={phone}
        ...
        <Article>
          ...
          <PositionBox>
            <Position name="백엔드" onClick={CheckPosition} state={position}>
              백엔드
            </Position>
            <Position
              name="프론트엔드"
            ...
            <Position name="디자인" onClick={CheckPosition} state={position}>
              디자인
            </Position>
          </PositionBox>
        </Article>
        ...
        <Article>
          <ArgreeBox>
            <Argree
              name="주의사항"
              src={precautions ? checkedBox : checkBox}
              text="위의 주의사항을 확인하였습니다"
              onClick={checking}
            />
            <Argree
              name="개인정보"
              src={privacy ? checkedBox : checkBox}
              text="개인 정보 수집 및 이용에 동의합니다 (모집 종료 후 개인정보는 자동으로 파기됩니다)"
              onClick={checking}
            />
          </ArgreeBox>
        </Article>
        <ButtonBox>
          <Button name="임시저장" onClick={isSave}>
            {submitCount >= 1 ? `잠시만 기다려주세요...` : `지원서 불러오기`}
          </Button>
          <Button name="제출하기" disabled={buttonState} onClick={handleClick}>
            {submitCount >= 1 ? `잠시만 기다려주세요...` : `공통문항 작성하기`}
          </Button>
        </ButtonBox>
      </Section>
    );
