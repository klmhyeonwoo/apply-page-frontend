...
export default function Design() {
  ...
  const userHardWork = useSelector(
    (state: TestState) => state.fetcher.userHardWork
  );
  const userKeyWord = useSelector(
    (state: TestState) => state.fetcher.userKeyWord
  );
  const userMostDeeplyWork = useSelector(
    (state: TestState) => state.fetcher.userMostDeeplyWork
  );

  const userWhyDesign = useSelector(
    (state: TestState) => state.fetcher.userWhyDesign
  );
  const userToolExperience = useSelector(
    (state: TestState) => state.fetcher.userToolExperience
  );
  const userTeamworkExperience = useSelector(
    (state: TestState) => state.fetcher.userTeamworkExperience
  );
  const userDesignGrowth = useSelector(
    (state: TestState) => state.fetcher.userDesignGrowth
  );
  const userPortfolioLink = useSelector(
    (state: TestState) => state.fetcher.userPortfolioLinkDesign
  );
  ...
  const Back = () => {
    ...
    dispatch(
      saveDesign({
        userWhyDesign: whyDesign,
        userToolExperience: toolExperience,
        userTeamworkExperience: teamworkExperience,
        userDesignGrowth: designGrowth,
        userPortfolioLinkDesign: portfolioLink,
      })
    );
    navigate("/common");
  };
  ...
  const Submit = async () => {
    ...
    if (time > endTime) {
      ...
      window.confirm("최중 제출을 하면 수정이 불가해요, 제출하시겠어요?")
    ) {
      setSubmitCount((prev) => prev + 1);
      axios
        .post(
  ...
  return (
    <Section>
      ...
      {complete ? (
        <Modal
          text={`${name}님의 소중한 지원서가 정상적으로 제출되었어요!`}
          imgSrc={completeImg}
          alt="최종제출"
        >
          <Button name="제출하기" onClick={TempHome}>
            메인 화면으로 이동
          </Button>
        </Modal>
      ) : null}
      {temp ? (
        <Modal
          text="지원하신 학번으로 지원서가 저장이 되었어요!"
          imgSrc={tempImg}
        >
          <Button name="임시저장" onClick={TempHome}>
            메인 화면으로 이동
          </Button>
          <Button name="제출하기" onClick={TempBack}>
            이어서 작성하기
          </Button>
        </Modal>
      ) : null}
      ...
      <Article>
        <InputTitle>
          디자인 트랙을 선택하게 된 이유를 구체적으로 서술해주세요
          <Require />{" "}
        </InputTitle>
        <TextAreaBox
      ...
      <Article>
        <InputTitle>
          피그마나 Adobe XD와 같은 목업 툴에 관련된 경험을 해본 적이 있다면 그
          경험에 대해 자세히 설명을 해주세요
          <Require />{" "}
        </InputTitle>
        <TextAreaBox
          ...
          name="경험"
          onChange={handleChange}
          value={toolExperience}
        />
        <WordLength>{toolExperience.length}</WordLength>
      </Article>
      <Article>
        <InputTitle>
          본인이 협업과 팀워크를 진행해 보았던 경험과, 그 경험을 멋쟁이 사자처럼
          대학에서 어떻게 적용시킬 수 있는지 알려주세요
          <Require />{" "}
        </InputTitle>
        <TextAreaBox
          placeholder="텍스트를 입력해주세요"
      ...
      <Article>
        <InputTitle>
          디자인 트랙을 통해 어떠한 성장을 희망하시는지 구체적으로 서술해주세요
          <Require />{" "}
        </InputTitle>
        <TextAreaBox
          placeholder="텍스트를 입력해주세요"
      ...
      <ButtonBox>
        <Button name="임시저장" onClick={TempSave} disabled={tempState}>
          {submitCount >= 1 ? `잠시만 기다려주세요...` : `임시저장`}
        </Button>
        <Button name="제출하기" onClick={Back}>
          {submitCount >= 1 ? `잠시만 기다려주세요...` : `뒤로가기`}
        </Button>
        <Button name="제출하기" onClick={Submit} disabled={buttonState}>
          {submitCount >= 1 ? `잠시만 기다려주세요...` : `제출하기`}
        </Button>
      </ButtonBox>
    </Section>
  );
