...
export default function Backend() {
  ...
  const userDifficultAndOvercoming = useSelector(
    (state: TestState) => state.fetcher.userDifficultAndOvercoming
  );
  const userStudyFramework = useSelector(
    (state: TestState) => state.fetcher.userStudyFramework
  );
  const userImportantGroup = useSelector(
    (state: TestState) => state.fetcher.userImportantGroup
  );
  const userPortfolioLink = useSelector(
    (state: TestState) => state.fetcher.userPortfolioLinkBack
  );
  ...
  const Back = () => {
    ...
    dispatch(
      saveBackEnd({
        userDifficultAndOvercoming: difficultAndOvercoming,
        userStudyFramework: studyFramework,
        userImportantGroup: importantGroup,
        userPortfolioLinkBack: portfolioLink,
      })
    );
    navigate("/common");
  };
  ...
  return (
    <Section>
      ...
      {temp ? (
        <Modal
          text="소중한 지원서가 학번으로 지원서가 저장이 되었어요!"
          imgSrc={tempImg}
        >
      ...
      <Article>
        <InputTitle>
          개발 관련 공부를 하며 개인적으로 힘들었던 경험과 그걸 극복했던
          자신만의 방법이 있나요?
          <Require />{" "}
        </InputTitle>
        <TextAreaBox
          ...
          value={difficultAndOvercoming}
        />
        <WordLength>{difficultAndOvercoming.length}</WordLength>
      </Article>
      <Article>
        <InputTitle>
          웹 백앤드 프레임워크를 공부해보신적 있으신가요? 있으시다면 어디까지
          공부해보셨나요?
          <Require />{" "}
        </InputTitle>
        <TextAreaBox
