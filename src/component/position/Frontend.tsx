...
export default function Frontend() {
  ...
  const userWhyFrontend = useSelector(
    (state: TestState) => state.fetcher.userWhyFrontend
  );
  const userUsingStack = useSelector(
    (state: TestState) => state.fetcher.userUsingStack
  );
  const userTeamProject = useSelector(
    (state: TestState) => state.fetcher.userTeamProject
  );
  const userAchieve = useSelector(
    (state: TestState) => state.fetcher.userAchieve
  );
  const userPortfolioLink = useSelector(
    (state: TestState) => state.fetcher.userPortfolioLinkFront
  );
  ...
  const Back = () => {
    ...
    dispatch(
      saveFrontEnd({
        userWhyFrontend: whyFrontend,
        userUsingStack: usingStack,
        userTeamProject: teamProject,
        userAchieve: achieve,
        userPortfolioLinkFront: portfolioLink,
      })
    );
    navigate("/common");
  };
  ...
  return (
    <Section>
      ...
      <Article>
        <InputTitle>
          프론트엔드 개발과 관련된 프레임워크나 HTML, CSS, JavaScript 등의
          언어를 사용해보신 경험에 대해 구체적으로 적어주세요
          <Require />{" "}
        </InputTitle>
        <TextAreaBox
          placeholder="텍스트를 입력해주세요"
      ...
      <Article>
        <InputTitle>
          팀 활동이나 프로젝트를 경험해본 내용과 이를 통해 자신의 성장 경험에
          대해서 서술해주세요
          <Require />{" "}
        </InputTitle>
        <TextAreaBox
