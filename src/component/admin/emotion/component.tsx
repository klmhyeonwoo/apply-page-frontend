...
export const FailMailSend = (props: MailType) => {
  return (
    ...
    >
      ...
      포지션의 불합격 메일 전체 전송을 하고싶으신가요?{" "}
    </div>
  );
};

export const PassMailSend = (props: MailType) => {
  const passMailList = useSelector(
    (state: TestState) => state.fetcher.passMailList
  );
...
export const List = React.memo(function List(props: ListType) {
  ...
  const ChangeCheckState: ChangeCheckStateType = useCallback(
  ...
  useEffect(() => {
    if (checkState) {
      ...
      dispatch(
        updatePassMailList(
          passMailList.filter((items: mailUserType) => {
      ...
      {props.check === "전송 체크" ? (
        ...
        !props.check && (
          ...
          >
            {" "}
            <img
              src={checkState ? checkedBox : checkBox}
              css={css`
                width: 1.2em;
              `}
            />{" "}
